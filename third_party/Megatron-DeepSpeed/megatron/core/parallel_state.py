...
_PIPELINE_MODEL_PARALLEL_GROUP_LIST = None
...
_DATA_PARALLEL_GROUP_LIST = None
...
# A worker might belong to one or more PP groups
_PIPELINE_GLOBAL_RANKS_LIST = None
...
# A list of lists of global ranks for each data parallel group to ease calculation of the source
# rank when broadcasting weights from src to all other data parallel ranks
# A worker might belong to one or more DP groups
_DATA_PARALLEL_GLOBAL_RANKS_LIST = None
...
def initialize_model_parallel_from_file(
    ...
    [stage_0_config, stage_1_config, ....] -> of size PP
    stage_i_config is [node_0, node_1, ....] -> of size DP
    node_j is of config [rank_0, rank_1, ....] and can differ in length
    ...
    assert len(config) == pipeline_model_parallel_size, "Difference in provided PP degree and PP degree found in config file"

    data_parallel_size: int = len(config[0])
    sequence_parallel_size = 1

    num_tensor_model_parallel_groups: int = data_parallel_size * pipeline_model_parallel_size
    # num_pipeline_model_parallel_groups: int = world_size // pipeline_model_parallel_size # TODO!
    #num_data_parallel_groups: int = world_size // data_parallel_size #TODO!
    num_sequence_parallel_groups: int = world_size // sequence_parallel_size
    #num_sequence_data_parallel_groups: int = world_size // sequence_parallel_size // data_parallel_size #TODO!

    tps_stage = [set() for _ in range(pipeline_model_parallel_size)] # set of the different tp for that stage
    for j,stage_config in enumerate(config):
        ranks = [i for node in stage_config for i in node]
    ...
    for j in range(pipeline_model_parallel_size):
        # assert that only {1,2,4} are possible values!
        for tp in tps_stage[j]:
            assert tp in {1,2,4}, "Only TP in {1,2,4} is allowed!"

        # find num of dp groups
        num_dp_groups_j = max(tps_stage[j])
        # do group partitioning
        for i in range(num_dp_groups_j):
            group_i = []
            group_i_list = []
            for node in config[j]:
                tp_this_node = len(node)
                node_offset = num_dp_groups_j // tp_this_node

                start_idx = i // node_offset
                end_idx = start_idx+1 #(i+1) // node_offset
                group_i += node[start_idx:end_idx]
                group_i_list.append(node[start_idx:end_idx])
            all_data_parallel_group_ranks.append(group_i)
            all_data_parallel_group_ranks_lists.append(group_i_list)


        # 2. Build all pipeline-parallel groups.
    all_pipeline_parallel_group_ranks = []
    all_pipeline_parallel_group_ranks_lists = []
    tps_pp_stage = [set() for _ in range(data_parallel_size)] # set of the different tp for that stage
    for i in range(data_parallel_size):
        stages = [stage_config[i] for stage_config in config]
        for node in stages:
            tps_pp_stage[i].add(len(node))

        tps_pp_stage[i] = list(tps_pp_stage[i])
        num_pp_groups_i = max(tps_pp_stage[i])
        # do group partitioning
        for j in range(num_pp_groups_i):
            group_j = []
            group_j_list = []
            for node in stages:
                tp_this_node = len(node)
                node_offset = num_pp_groups_i // tp_this_node

                start_idx = j // node_offset
                end_idx = start_idx+1
                group_j += node[start_idx:end_idx]
                group_j_list.append(node[start_idx:end_idx])
            all_pipeline_parallel_group_ranks.append(group_j)
            all_pipeline_parallel_group_ranks_lists.append(group_j_list)

    # print(f"all_pipeline_parallel_group_ranks is {all_pipeline_parallel_group_ranks}")
    # print(f"all_pipeline_parallel_group_ranks_lists is {all_pipeline_parallel_group_ranks_lists}")

    # 3. Find data-parallel group of this rank.
    global _DATA_PARALLEL_GROUP_LIST
    global _DATA_PARALLEL_GROUP_GLOO
    global _DATA_PARALLEL_GLOBAL_RANKS_LIST
    assert _DATA_PARALLEL_GROUP_LIST is None, 'data parallel group is already initialized'
    _DATA_PARALLEL_GROUP_GLOO = None
    ...
    # print(f"all_data_parallel_group_ranks is {all_data_parallel_group_ranks}")
    # print(f"all_data_parallel_group_ranks_lists is {all_data_parallel_group_ranks_lists}")
    # print(f"_DATA_PARALLEL_GROUP_LIST is {_DATA_PARALLEL_GROUP_LIST}, _DATA_PARALLEL_GLOBAL_RANKS_LIST is {_DATA_PARALLEL_GLOBAL_RANKS_LIST}")

    # # 4. Build the sequence parallel groups.
    ...
    # 5. Build the sequence data parallel groups.
    ...
    _SEQUENCE_DATA_PARALLEL_GROUP = _DATA_PARALLEL_GROUP_LIST[0]

        # # 6. Build the model-parallel groups.
    global _MODEL_PARALLEL_GROUP
    assert _MODEL_PARALLEL_GROUP is None, 'model parallel group is already initialized'
    num_model_parallel_groups = data_parallel_size
    model_parallel_group_ranks = all_data_parallel_group_ranks_lists
    for i in range(num_model_parallel_groups):
        ranks_list = [parallel_group_ranks[i] for parallel_group_ranks in model_parallel_group_ranks]
        ranks_set = set()
        for l in ranks_list:
            for r in l:
                ranks_set.add(r)
    ...
    # # 7. Build the tensor model-parallel groups.
    global _TENSOR_MODEL_PARALLEL_GROUP
    global _TENSOR_MODEL_PARALLEL_RANKS
    ...
    # 8. Build the pipeline model-parallel groups and embedding groups
    # (first and last rank in each pipeline model-parallel group).
    global _PIPELINE_MODEL_PARALLEL_GROUP_LIST
    global _PIPELINE_GLOBAL_RANKS_LIST
    assert _PIPELINE_MODEL_PARALLEL_GROUP_LIST is None, 'pipeline model parallel group is already initialized'
    ...
    # EMBEDDING GROUPS - TODO: Check this!
    for ranks in all_pipeline_parallel_group_ranks:
    ...
    # 9. Build the FP8 groups.
    global _AMAX_REDUCTION_GROUP
    assert _AMAX_REDUCTION_GROUP is None, \
        'FP8 amax reduction group is already initialized'

    print(f"================ RANK {rank}, _DATA_PARALLEL_GLOBAL_RANKS_LIST: {_DATA_PARALLEL_GLOBAL_RANKS_LIST}")
    print(f"================ RANK {rank}, _PIPELINE_GLOBAL_RANKS_LIST: {_PIPELINE_GLOBAL_RANKS_LIST}")
    print(f"================ RANK {rank}, _TENSOR_MODEL_PARALLEL_RANKS: {_TENSOR_MODEL_PARALLEL_RANKS}")
    print(f"================ RANK {rank}, _EMBEDDING_GLOBAL_RANKS: {_EMBEDDING_GLOBAL_RANKS}")
    print(f"================ RANK {rank}, _POSITION_EMBEDDING_GLOBAL_RANKS: {_POSITION_EMBEDDING_GLOBAL_RANKS}")
...
def initialize_model_parallel(
    ...
    _EMBEDDING_GLOBAL_RANKS = list(_EMBEDDING_GLOBAL_RANKS)
    _POSITION_EMBEDDING_GLOBAL_RANKS = list(_POSITION_EMBEDDING_GLOBAL_RANKS)
    _DATA_PARALLEL_GLOBAL_RANKS_LIST = [list(x) for x in _DATA_PARALLEL_GLOBAL_RANKS_LIST]
    _PIPELINE_GLOBAL_RANKS_LIST = [list(x) for x in _PIPELINE_GLOBAL_RANKS_LIST]
    _TENSOR_MODEL_PARALLEL_RANKS = list(_TENSOR_MODEL_PARALLEL_RANKS)
    print(f"**************** RANK {rank}, _DATA_PARALLEL_GLOBAL_RANKS_LIST: {_DATA_PARALLEL_GLOBAL_RANKS_LIST}")
    print(f"**************** RANK {rank}, _PIPELINE_GLOBAL_RANKS_LIST: {_PIPELINE_GLOBAL_RANKS_LIST}")
    print(f"**************** RANK {rank}, _TENSOR_MODEL_PARALLEL_RANKS: {_TENSOR_MODEL_PARALLEL_RANKS}")
    print(f"**************** RANK {rank}, _EMBEDDING_GLOBAL_RANKS: {_EMBEDDING_GLOBAL_RANKS}")
...
def is_unitialized():
    ...
    return _DATA_PARALLEL_GROUP_LIST is None


def model_parallel_is_initialized():
    """Check if model and data parallel groups are initialized."""
    if _TENSOR_MODEL_PARALLEL_GROUP is None or _PIPELINE_MODEL_PARALLEL_GROUP_LIST is None or _DATA_PARALLEL_GROUP_LIST is None:
        return False
    return True
...
def sequence_parallel_is_initialized():
    ...
    if _SEQUENCE_PARALLEL_GROUP is None or \
        _DATA_PARALLEL_GROUP_LIST is None:
        return False
    return True
...
def get_all_data_parallel_group_ranks():
...
def get_pipeline_model_parallel_group():
    ...
    assert _PIPELINE_MODEL_PARALLEL_GROUP_LIST is not None, 'pipeline_model parallel group is not initialized'
    return _PIPELINE_MODEL_PARALLEL_GROUP_LIST[0]
...
def get_data_parallel_group():
    ...
    assert _DATA_PARALLEL_GROUP_LIST is not None, 'data parallel group is not initialized'
    return _DATA_PARALLEL_GROUP_LIST[0]
...
def get_tensor_model_parallel_src_rank():
    ...
    assert _TENSOR_MODEL_PARALLEL_GROUP is not None, 'tensor model parallel group is not initialized'
    all_ranks = torch.distributed.get_process_group_ranks(_TENSOR_MODEL_PARALLEL_GROUP)
    return all_ranks[0]
...
def get_data_parallel_src_rank():
    ...
    return _DATA_PARALLEL_GLOBAL_RANKS_LIST[0]
...
def get_pipeline_model_parallel_first_rank():
    ...
    return _PIPELINE_GLOBAL_RANKS_LIST[0]
...
def get_pipeline_model_parallel_last_rank():
    ...
    return _PIPELINE_GLOBAL_RANKS_LIST[0][last_rank_local]
...
def get_pipeline_model_parallel_next_rank():
    ...
    return _PIPELINE_GLOBAL_RANKS_LIST[0][(rank_in_pipeline + 1) % world_size]
